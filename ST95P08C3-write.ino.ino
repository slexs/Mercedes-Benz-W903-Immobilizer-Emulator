...
Installing this emulator in the Mercedes Benz W903 Engine Control Unit will let your circumvent any problems related to the Immobilizer Unit / WSP. 
...
void setup() {
  ...
  // Optinally: Read all data from all addresses (Might be nice to start with this so you have a backup of OEM data)
  readEEPROMAll();
...
void readEEPROMAll() {
    ...
    for (unsigned int startAddress = 0; startAddress < totalMemory; startAddress += chunkSize) {
        ...
        for (byte i = 0; i < chunkSize; i++) {
            ...
            delay(10); // Introducing a small delay between each byte
        }
